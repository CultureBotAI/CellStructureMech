...
from cellstructuremech import text_map_site as site
...
def test_navigation_is_conditional_and_uses_relative_site_root():
    templates = Path(__file__).resolve().parents[1] / "src" / "cellstructuremech" / "templates"
...
def test_landing_exposes_enabled_map_and_retains_legacy_view():
    templates = Path(__file__).resolve().parents[1] / "src" / "cellstructuremech" / "templates"
    env = Environment(loader=FileSystemLoader(str(templates)))
    template = env.get_template("index.html")
    context = {"root": "", "categories": [], "total": 306, "grounded": 253}
    disabled = template.render(text_map_enabled=False, **context)
    enabled = template.render(text_map_enabled=True, **context)
    assert 'href="text-map/"' not in disabled
    assert 'href="text-map/"' in enabled
    assert "BGE" in enabled and "PaCMAP" in enabled
    assert 'href="embedding-map.html"' in enabled
    assert "MiniLM" in enabled and "PCA" in enabled
