# Merging pull requests in CellStructureMech
...
3. Use the PR's merge control to enter the queue. From the CLI, run
   `gh pr merge <number> --repo CultureBotAI/CellStructureMech` after those checks pass.
...
- `identifier-liveness` from `.github/workflows/identifier-liveness.yaml`.
