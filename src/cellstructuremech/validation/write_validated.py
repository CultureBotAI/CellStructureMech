"""Write-time validation: dump a CellStructureRecord to YAML *only if* it passes
...
Ported from HabitatMech / TraitMech ``validation/write_validated.py``.
"""
...
DEFAULT_SCHEMA_PATH = Path(__file__).resolve().parents[1] / "schema" / "cellstructuremech.yaml"
DEFAULT_TARGET_CLASS = "CellStructureRecord"
...
def emit_structure_yaml(doc: dict[str, Any], yaml_kwargs: dict[str, Any] | None = None) -> str:
    """Serialise ``doc`` exactly as :func:`write_validated_structure` writes it."""
...
def write_validated_structure(
    ...
    *,
    ...
    """
    errors = validate_structure(doc, target_class=target_class, schema_path=schema_path)
    if errors:
        raise ValidationFailedError(path, errors)
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(emit_structure_yaml(doc, yaml_kwargs), encoding="utf-8")
