...
ADAPTER_VERSION = "cellstructuremech-semantic-v1"
CORPUS = "structures"
...
CATEGORY_FIELD = "structure_category"
...
def semantic_text(record: dict, context: Mapping[str, str] | None = None) -> str:
    # Text fields are shared; the encoder/profile/vector cache is not. BGE
    # inference must produce new 1,024D vectors, never reuse legacy MiniLM ones.
    from cellstructuremech.semantic_text import semantic_text as structure_text

    return structure_text(record)


def page_target(record: dict, source_path: str) -> str:
    page = Path(source_path).relative_to("data/structures").with_suffix(".html")
    return "structures/" + quote(page.as_posix(), safe="/")
...
def iter_inputs(
    ...
    with tempfile.TemporaryDirectory(prefix="cellstructuremech-map-inputs-") as directory:
