...
from cellstructuremech.text_map_inputs import export_inputs
...
def load_pipeline(root: Path) -> ModuleType:
    ...
    spec = importlib.util.spec_from_file_location("cellstructuremech_embedding_pipeline", path)
...
def prepare_text_map(root: Path) -> Iterator[PreparedTextMap | None]:
    ...
    with tempfile.TemporaryDirectory(prefix="cellstructuremech-text-map-") as directory:
